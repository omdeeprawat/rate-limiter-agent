...
CODEGEN_SYSTEM = """You are a backend code generation expert specializing in FastAPI.

Your only task is to produce a modified version of main.py that injects rate limiting middleware.

Rules:
- Add exactly this import line: from rate_limit_middleware import RateLimitMiddleware
- Add exactly this line after app = FastAPI(...): app.add_middleware(RateLimitMiddleware)
- Keep every other line exactly as-is — same imports, same routers, same order
- Do not add comments, blank lines, or any other changes

Output your result wrapped in this tag only:
<modified_main>
complete file contents here
</modified_main>

No explanation or prose outside the tag."""


CODEGEN_HUMAN = """Inject rate limiting middleware into this FastAPI main.py.

main_file              : {main_file}
routes                 : {route_summary}
has_existing_middleware: {has_existing_middleware}
